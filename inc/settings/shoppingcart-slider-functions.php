...
function child_shoppingcart_category_sliders() {
	...
	if($query->have_posts() && !empty($category)){
		...
		while ($query->have_posts()):$query->the_post();
				...
				$category_sliders_display 	.= '<div>';	
				$rating = wc_get_product( get_the_ID() )->get_average_rating();
				$aa = '';
				for($i= 1; $i<=$rating; $i++) {
					$aa = $aa . '<i class="fa fa-star star-rating-hp" aria-hidden="true"></i>';
				}

				$category_sliders_display 	.=	$aa;
